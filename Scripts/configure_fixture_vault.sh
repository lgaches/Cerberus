...
vault secrets enable -version=1 kv > /dev/null
vault secrets move kv secret > /dev/null

ACCESSIBLE_SECRET_PATH=cerberus # defined in `narrow_policy.hcl`
...
vault kv put secret/$ACCESSIBLE_SECRET_PATH secret="this is a very secret thing but it is the only thing you can read" > /dev/null
